		...
		//for (int i = 0; i <= SCREEN_WIDTH/ROOM_SIZE; i++) {
			...
			//rooms[i][j].setMissingWall(bottom);
				...
				if (//side cases
				  ...
				  (rooms[i  ][j-1].isVisited() == 1 && rooms[i-1][j  ].isVisited() == 1 && rooms[i+1][j  ].isVisited() == 1 && j == SCREEN_HEIGHT/ROOM_SIZE) ||
				  (rooms[i  ][j-1].isVisited() == 1 && rooms[i  ][j+1].isVisited() == 1 && rooms[i-1][j  ].isVisited() == 1 && i == SCREEN_WIDTH/ROOM_SIZE) || 
				  //corner cases
				  (rooms[i  ][j+1].isVisited() == 1 && rooms[i+1][j  ].isVisited() == 1 && i == 0 && j == 0) ||
				  (rooms[i  ][j+1].isVisited() == 1 && rooms[i-1][j  ].isVisited() == 1 && i == SCREEN_WIDTH/ROOM_SIZE && j == 0) || 
				  (rooms[i  ][j-1].isVisited() == 1 && rooms[i+1][j  ].isVisited() == 1 && i == 0 && j == SCREEN_HEIGHT/ROOM_SIZE) ||
				  (rooms[i  ][j-1].isVisited() == 1 && rooms[i-1][j  ].isVisited() == 1 && i == SCREEN_WIDTH/ROOM_SIZE && j == SCREEN_HEIGHT/ROOM_SIZE)) {
					SDL_Point prev = rooms[i][j].getPrev();
					i = prev.x;
					j = prev.y;
