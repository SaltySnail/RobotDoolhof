...
void model::path::setWeightFinish(SDL_Point finish) {
	int tmp = INT_MAX/2;	
	nodes[finish.x-1][finish.y-1].setWeightOpening(&tmp, top); 
	nodes[finish.x-1][finish.y-1].setWeightOpening(&tmp, left);
	nodes[finish.x-1][finish.y-1].setWeightOpening(&tmp, right);
	tmp = 0;
	nodes[finish.x-1][finish.y-1].setWeightOpening(&tmp, bottom); 
}

bool model::path::isFinished(SDL_Point counter) {
	if (counter.x+2 == SCREEN_WIDTH/ROOM_SIZE && counter.y+2 == SCREEN_WIDTH/ROOM_SIZE) {
		return true;
	}
	return false;
}
